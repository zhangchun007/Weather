...
public abstract class BaseCalendar extends ViewPager implements ICalendar {
    ...
    private boolean mDefaultCheckedFirstDate = true;//默认选择时，翻页选中第一个日期
    ...
    private void drawView(int position) {
        ...
        if (mCheckModel == CheckModel.SINGLE_DEFAULT_CHECKED && mDateChangeBehavior == DateChangeBehavior.PAGE) {
            ...
            if (this instanceof MonthCalendar) {
                if (mDefaultCheckedFirstDate && tempLocalDate.monthOfYear().get() == LocalDate.now().monthOfYear().get() && tempLocalDate.year().get() == LocalDate.now().year().get()) {
                    currentDate = LocalDate.now();
                } else if (mDefaultCheckedFirstDate && (((lastDate.year().get() == tempLocalDate.year().get()) && lastDate.monthOfYear().get() < tempLocalDate.monthOfYear().get()) || lastDate.year().get() < tempLocalDate.year().get())) {
                ...
                LocalDate lastDate1 = getLastDate();
                LocalDate localDateNow = LocalDate.now();
                if (CalendarUtil.isEqualsDay(firstDate, localDateNow) || CalendarUtil.isEqualsDay(lastDate1, localDateNow) || (localDateNow.isBefore(lastDate1) && localDateNow.isAfter(firstDate))) {
                    currentDate = LocalDate.now();
                } else if (lastDate.isBefore(tempLocalDate)) {
                    currentDate = getFirstDate();
                } else if (lastDate.isAfter(tempLocalDate)) {
                    currentDate = getLastDate();
                } else {
                    currentDate = tempLocalDate;
                }
    ...
    protected void jump(LocalDate localDate, boolean isCheck, DateChangeBehavior dateChangeBehavior) {
        ...
        if (!isAvailable(localDate)) {
            ...
            int indexOffset = 0;
            if (iCalendarView != null){
                indexOffset    = getTwoDateCount(localDate, iCalendarView.getPagerInitialDate(), mFirstDayOfWeek);
            }
            if (isCheck) {
    ...
    public void jumpDate(LocalDate localDate) {
        if (localDate == null) {
            localDate = LocalDate.now();
        }
        jump(localDate, true, DateChangeBehavior.API);
    }
