...
public class SolarTermUtil {
    ...
    static {
        DECREASE_OFFSETMAP.put(SolarTermsEnum.JINGZHE.name(), new Integer[]{1915,
                1948, 2000});// 惊蛰 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.CHUNFEN.name(),
                new Integer[]{2000});// 春分 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.YUSHUI.name(),
                new Integer[]{1923, 2026});// 雨水
        DECREASE_OFFSETMAP.put(SolarTermsEnum.LIXIA.name(),
                new Integer[]{2000});// 立夏 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.MANGZHONG.name(),
                new Integer[]{2000});// 芒种 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.XIAZHI.name(),
                new Integer[]{2000});// 夏至 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.DASHU.name(),
                new Integer[]{2000});// 大暑 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.LIQIU.name(),
                new Integer[]{2000});// 立秋 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.HANLU.name(),
                new Integer[]{2000});// 寒露 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.SHUANGJIANG.name(),
                new Integer[]{2000});// 霜降 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.LIDONG.name(),
                new Integer[]{2000});// 立冬 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.XIAOXUE.name(),
                new Integer[]{2000});// 小雪 -1
        DECREASE_OFFSETMAP.put(SolarTermsEnum.BAILU.name(),
                new Integer[]{2000});// 白露 -1
...
//                new Integer[]{2084});// 春分
        ...
        INCREASE_OFFSETMAP.put(SolarTermsEnum.LIDONG.name(),
                new Integer[]{2089});// 立冬
        INCREASE_OFFSETMAP.put(SolarTermsEnum.YUSHUI.name(),
                new Integer[]{1928, 1932, 1936, 1940, 1944, 1948, 1952, 1956, 2064, 2068, 2072, 2076, 2080, 2084, 2088, 2092});// 雨水
        INCREASE_OFFSETMAP.put(SolarTermsEnum.XIAOXUE.name(),
                new Integer[]{1978});// 小雪
//        INCREASE_OFFSETMAP.put(SolarTermsEnum.DAXUE.name(),
//                new Integer[]{1954});// 大雪
        INCREASE_OFFSETMAP.put(SolarTermsEnum.LICHUN.name(),
                new Integer[]{1952, 1956, 1960, 1964, 1968, 1972, 1976, 1980, 2084, 2088, 2092, 2096});// 立春
        INCREASE_OFFSETMAP.put(SolarTermsEnum.DONGZHI.name(),
                new Integer[]{1914, 1947, 1951, 1980, 1984});// 冬至
        INCREASE_OFFSETMAP.put(SolarTermsEnum.LIXIA.name(),
                new Integer[]{1911});// 立夏

        DECREASE_OFFSETMAP.put(SolarTermsEnum.DONGZHI.name(), new Integer[]{
                2000, 2021});// 冬至
        DECREASE_OFFSETMAP.put(SolarTermsEnum.LICHUN.name(), new Integer[]{
                1947});// 立春 -1

        DECREASE_OFFSETMAP.put(SolarTermsEnum.XIAOHAN.name(), new Integer[]{
                2019});// 小寒 -1

        INCREASE_OFFSETMAP.put(SolarTermsEnum.XIAOHAN.name(),
                new Integer[]{1904, 1908, 1912, 1982, 2000, 2024, 2028, 2032, 2036, 2040, 2044, 2048});// 小寒

        INCREASE_OFFSETMAP.put(SolarTermsEnum.DAHAN.name(),
                new Integer[]{1980, 1984, 1988, 1992, 1996, 2000, 2004, 2008, 2082, 2012});// 大寒
    }
    ...
    public static String[] term = new String[]{"小寒","大寒","立春","雨水","惊蛰","春分","清明","谷雨","立夏","小满","芒种","夏至","小暑","大暑","立秋","处暑","白露"
    ,"秋分","寒露","霜降","立冬","小雪","大雪","冬至"};

    public static Map<LocalDate, String> getsolarTermToString(int year) {
        Map<LocalDate, String> map = new LinkedHashMap<>();

        // 23
        map.put(LocalDate.parse(year + "-" + 1 + "-" + getSolarTermNum(year, SolarTermsEnum.XIAOHAN.name())), "小寒");
        // 24
        map.put(LocalDate.parse(year + "-" + 1 + "-" + getSolarTermNum(year, SolarTermsEnum.DAHAN.name())), "大寒");
        // 1
        map.put(LocalDate.parse(year + "-" + 2 + "-" + getSolarTermNum(year, SolarTermsEnum.LICHUN.name())), "立春");
        // 2
        map.put(LocalDate.parse(year + "-" + 2 + "-" + getSolarTermNum(year, SolarTermsEnum.YUSHUI.name())), "雨水");
        // 3
        map.put(LocalDate.parse(year + "-" + 3 + "-" + getSolarTermNum(year, SolarTermsEnum.JINGZHE.name())), "惊蛰");
        // 4
        map.put(LocalDate.parse(year + "-" + 3 + "-" + getSolarTermNum(year, SolarTermsEnum.CHUNFEN.name())), "春分");
        // 5
        map.put(LocalDate.parse(year + "-" + 4 + "-" + getSolarTermNum(year, SolarTermsEnum.QINGMING.name())), "清明");
        // 6
        map.put(LocalDate.parse(year + "-" + 4 + "-" + getSolarTermNum(year, SolarTermsEnum.GUYU.name())), "谷雨");
        // 7
        map.put(LocalDate.parse(year + "-" + 5 + "-" + getSolarTermNum(year, SolarTermsEnum.LIXIA.name())), "立夏");
        // 8
        map.put(LocalDate.parse(year + "-" + 5 + "-" + getSolarTermNum(year, SolarTermsEnum.XIAOMAN.name())), "小满");
        // 9
        map.put(LocalDate.parse(year + "-" + 6 + "-" + getSolarTermNum(year, SolarTermsEnum.MANGZHONG.name())), "芒种");
        // 10
        map.put(LocalDate.parse(year + "-" + 6 + "-" + getSolarTermNum(year, SolarTermsEnum.XIAZHI.name())), "夏至");
        // 11
        map.put(LocalDate.parse(year + "-" + 7 + "-" + getSolarTermNum(year, SolarTermsEnum.XIAOSHU.name())), "小暑");
        // 12
        map.put(LocalDate.parse(year + "-" + 7 + "-" + getSolarTermNum(year, SolarTermsEnum.DASHU.name())), "大暑");
        // 13
        map.put(LocalDate.parse(year + "-" + 8 + "-" + getSolarTermNum(year, SolarTermsEnum.LIQIU.name())), "立秋");
        // 14
        map.put(LocalDate.parse(year + "-" + 8 + "-" + getSolarTermNum(year, SolarTermsEnum.CHUSHU.name())), "处暑");
        // 15
        map.put(LocalDate.parse(year + "-" + 9 + "-" + getSolarTermNum(year, SolarTermsEnum.BAILU.name())), "白露");
        // 16
        map.put(LocalDate.parse(year + "-" + 9 + "-" + getSolarTermNum(year, SolarTermsEnum.QIUFEN.name())), "秋分");
        // 17
        map.put(LocalDate.parse(year + "-" + 10 + "-" + getSolarTermNum(year, SolarTermsEnum.HANLU.name())), "寒露");
        // 18
        map.put(LocalDate.parse(year + "-" + 10 + "-" + getSolarTermNum(year, SolarTermsEnum.SHUANGJIANG.name())), "霜降");
        // 19
        map.put(LocalDate.parse(year + "-" + 11 + "-" + getSolarTermNum(year, SolarTermsEnum.LIDONG.name())), "立冬");
        // 20
        map.put(LocalDate.parse(year + "-" + 11 + "-" + getSolarTermNum(year, SolarTermsEnum.XIAOXUE.name())), "小雪");
        // 21
        map.put(LocalDate.parse(year + "-" + 12 + "-" + getSolarTermNum(year, SolarTermsEnum.DAXUE.name())), "大雪");
        // 22
        map.put(LocalDate.parse(year + "-" + 12 + "-" + getSolarTermNum(year, SolarTermsEnum.DONGZHI.name())), "冬至");
