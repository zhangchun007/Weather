...
public class CalendarHelper {
    ...
    private int marginlr = DensityUtil.dp2px(7);

    private RectF resetRectFSize(RectF rectF, int lineIndex, int columnIndex) {


        //矩形重新确定确定位置
        float width = mCalendar.getMeasuredWidth() - marginlr * 2;
        float height = mCalendar.getMeasuredHeight();
        float rectWidth = width / 7;
        //为每个矩形确定位置
        if (mLineNum == 5 || mLineNum == 1) {
            //5行的月份，5行矩形平分view的高度  mLineNum==1是周的情况
            float rectHeight = height / mLineNum;
            float left = columnIndex * width / 7 + marginlr;
            float top = lineIndex * rectHeight;
            float right = left + rectWidth;
            float bottom = lineIndex * rectHeight + rectHeight;
            ...
            float left = columnIndex * width / 7 + marginlr;
            float top = lineIndex * rectHeight6 + (rectHeight5 - rectHeight6) / 2;
            float right = left + rectWidth;
            float bottom = lineIndex * rectHeight6 + rectHeight6 + (rectHeight5 - rectHeight6) / 2;
    ...
    public LocalDate getCurrPagerLastDate() {
        if (mCalendarType == CalendarType.MONTH) {
            return new LocalDate(mPagerInitialDate.getYear(), mPagerInitialDate.getMonthOfYear(), CalendarUtil.getDaysOfMonth(mPagerInitialDate));
        } else {
            return mDateList.get(mDateList.size() - 1);
        }
    }
