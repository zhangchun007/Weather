...
public class NumberPickerViewButtom extends View {
    ...
    private int mTextColorNormalTwo = DEFAULT_TEXT_COLOR_NORMAL;
    ...
    private TextPaint mPaintText = new TextPaint(Paint.FAKE_BOLD_TEXT_FLAG);
    ...
    public interface OnValueChangeListener {
        void onValueChange(NumberPickerViewButtom picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw {
        void onValueChangeRelativeToRaw(NumberPickerViewButtom picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling {
        void onValueChangeInScrolling(NumberPickerViewButtom picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(NumberPickerViewButtom view, int scrollState);
    }
    ...
    public NumberPickerViewButtom(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init(context);
    }

    public NumberPickerViewButtom(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void respondPickedValueChangedInScrolling(int oldVal, int newVal) {
        mOnValueChangeListenerInScrolling.onValueChangeInScrolling(NumberPickerViewButtom.this, oldVal, newVal);
    }
        ...
        if (currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach) {
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerViewButtom.this, mPrevPickedIndex + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(NumberPickerViewButtom.this, mPrevPickedIndex, currPickedIndex, mDisplayedValues);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        ...
        if (oldVal != newVal) {
            if (respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerViewButtom.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(NumberPickerViewButtom.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
    ...
    public void setNormalTextColorTwo(int normalTextColor) {
        if (mTextColorNormalTwo == normalTextColor) {
            return;
        }
        mTextColorNormalTwo = normalTextColor;
        postInvalidate();
    }
    ...
    public void setDividerColor(int dividerColor) {
        ...
        mPaintDivider.setColor(Color.parseColor("#EEEEEE"));
        postInvalidate();
    }
    ...
    private void onScrollStateChange(int scrollState) {
        ...
        if (mOnScrollListener != null) {
            mOnScrollListener.onScrollStateChange(NumberPickerViewButtom.this, scrollState);
        }
    }
    ...
    private void drawContent(Canvas canvas) {
        ...
        for (int i = 0; i < mShownCount + 1; i++) {
            ...
            if (i == mShownCount / 2) {//this will be picked
                ...
                fraction =    (float) (mItemHeight + mCurrDrawFirstItemY) / mItemHeight;
                textColor = getEvaluateColor( fraction, mTextColorNormalTwo, mTextColorNormal);
           //     textColor = mTextColorNormalTwo;
                textSize = mTextSizeNormal;
                textSizeCenterYOffset = mTextSizeNormalCenterYOffset;
            } else if (i == mShownCount / 2 + 2) {
                textColor = getEvaluateColor(1 - fraction, mTextColorNormalTwo, mTextColorNormal);
                textSize = mTextSizeNormal;
                textSizeCenterYOffset = mTextSizeNormalCenterYOffset;
            } else if (i == mShownCount / 2 - 2){
                textColor = mTextColorNormalTwo;
