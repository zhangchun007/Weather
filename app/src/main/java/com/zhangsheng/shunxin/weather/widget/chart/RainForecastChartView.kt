...
class RainForecastChartView @JvmOverloads constructor(
    context: Context,
    ...
    private fun initPain() {
        ...
        chartLinePaint.color = Color.parseColor("#66ffffff")
        chartLinePaint.strokeWidth = dip2px(0.5f)

        chartPaint = Paint()
        chartPaint.strokeJoin = Paint.Join.ROUND
        chartPaint.strokeWidth = dip2px(0.5f)
        chartPaint.isAntiAlias = true
        chartPaint.isDither = true   //防抖动
        chartPaint.style = Paint.Style.STROKE
        chartPaint.color = Color.parseColor("#99ffffff")


        textPaint = Paint()
        textPaint.color = Color.parseColor("#bfffffff")
        textPaint.textSize = dip2px(10f)

        shadowPaint = Paint()
        shadowPaint.style = Paint.Style.FILL
        shadowPaint.strokeWidth = dip2px(4f)
        shadowPaint.color = Color.parseColor("#40ffffff")
        shadowPaint.isAntiAlias = true
        shadowPaint.isDither = true   //防抖动

        yTvPaint = Paint()
        yTvPaint.color = Color.parseColor("#bfffffff")
        yTvPaint.textSize = dip2px(10f)
    ...
    fun setWhiteTheme() {
        ...
        shadowPaint.color = Color.parseColor("#409FDCFF")
        postInvalidate()
    }
    ...
    private fun drawChartBaseLine(canvas: Canvas?) {
        canvas?.drawLine(lrPadding,0f,lrPadding,distanceY*3,chartLinePaint)

        canvas?.drawLine(width.toFloat(),0f,width.toFloat(),distanceY*3,chartLinePaint)
        canvas?.let { canvas ->
            (1..3).forEach {
                canvas.drawLine(
                    lrPadding, distanceY * it,
                    width.toFloat(), distanceY * it, chartLinePaint
                )
            }
        }
