...
class WeatherPageFragment : AacFragment<WeatherPageViewModel,FragmentWeatherPageBinding>(R.layout.fragment_weather_page),
    ...
    override fun initView() {
        changePageHeight()
        binding.vBottom.background.mutate().alpha = 0
        ...
        binding.calendar.ClickReport(EnumType.上报埋点.生活指数模块日历点击) {
            if (!isControl()) {
    ...
    override fun initObserve() {
        ...
        getAppModel().control.safeObserve(this, Observer {
            if (!isControlShow(
                    getAppModel().control.value.nN().swtich_all.listIndex(
                        0
                    ).infostream
                )
            ) {
        ...
        vm.weatherData.safeObserve(this, Observer {
            binding.layoutTopWeather.setWeatherDate(it)
            binding.layoutWeatherMiddle.setWeatherDate(it)
    ...
    private fun loadMoreWeather(it: WeatherBean) {
        initCalendar()
        binding.fifLayout.setWeatherDate(it)
...
//            WeatherFragment()
    ...
    private fun setErrorState(it: String) {
        Try {
            if (it.isEmpty()) {
                ...
                binding.layoutTopWeather.isVisible(false)
                binding.scrollview.isVisible(false)
            } else {
                binding.scrollview.isVisible(true)
                binding.layoutTopWeather.isVisible(true)
    ...
    private fun loadPageOneAd() {
        binding.layoutAd.loadAd()
        binding.layoutTopWeather.loadBannerAdv()
    ...
    private fun loadAd(scrollY: Int) {
        ...
        if (binding.fifLayout.getGlobalVisibleRect(Rect())) {
    ...
    override fun onScroll(scrollY: Int, oldScrollY: Int) {
        ...
        var alpha = scrollY.toFloat() / bgHalfHeight
        binding.vBottom.apply {
            this.background.mutate().alpha =
                (255 * if (alpha > 1) 1f else alpha).toInt()
            binding.llPageTow.background.mutate().alpha =
                (255 * if (alpha > 1) 1f else alpha).toInt()
            binding.bgTop.alpha = if (alpha > 1) 1f else alpha
        }

        if (scrollY <= 0){
            binding.layoutTopWeather.translationY = 0f
        }
        getParent()?.scrollChange(vm.index, scrollY,"onScroll:${vm.index}")
        loadAd(scrollY)
        if (shouldLoadInfo() && isControlShow(
                getAppModel().control.value.nN().swtich_all.listIndex(
                    0
                ).infostream
            )
