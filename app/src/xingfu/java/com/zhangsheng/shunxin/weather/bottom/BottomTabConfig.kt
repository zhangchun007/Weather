...
class BottomTabConfig {
    ...
    companion object {
        // 底部tab默认配置
        val bottomTabConfigs = arrayOf(BottomBarItem.CMD_WEATHER, BottomBarItem.CMD_STREAM, BottomBarItem.CMD_CALENDAR, BottomBarItem.CMD_TOOLBOX)
        ...
        fun createBottomBarItem(cmd: String) : BottomBarItem? {
            ...
            when(cmd) {
                ...
                BottomBarItem.CMD_STREAM -> {
                    item = BottomBarItem()
                    item.setTitle("资讯")
                    item.setTopDrawableId(R.drawable.select_tab_bt_news)
                    item.setEventCMD(cmd)
